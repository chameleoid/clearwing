...
Clearwing.prototype.emit = function() {
	...
	prop = Clearwing.stringifyPropName(ns.concat(event));
	...
	ns.forEach(function(item) {
		...
		switch (prop) {
			case 'network': data.network = that.network(name); break;
			case 'user': data.user = data.network.user(name); break;
			case 'channel': data.channel = data.network.channel(name); break;
		}
	});
