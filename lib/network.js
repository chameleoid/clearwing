...
function Network(client, name) {
	...
	/* set up events */
	var _line = '';
	this.on('socket:data', function(data) {
		data = (_line + data.message).split('\r\n');
		_line = data.pop();

		data.forEach(function(line) {
			var data    = that.parseMessage(line),
			    command = data.command.toLowerCase();

			that.emit('raw:' + command, data);
		});
	});

	this.on('socket:connect', function(event) {
		event.network
			.set('connected', true)
	...
	this.on('raw:001', function(event) {
		event.network
			.set('status', 'connected')
			.emit('connected', _.clone(event));
	});

	this.on('raw:ping', function(event) {
		event.network.raw('PONG :' + event.params.join(' '), false);
	});

	this.on('raw:mode', function(event) {
		var target = event.params[0];

		if (/#/.test(target))
			target = event.network.channel(target);
		else
			target = event.network.user(target);

		target.emit('raw:mode', _.clone(event));
	});
}
